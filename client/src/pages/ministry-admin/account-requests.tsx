...
export default function MinistryAdminAccountRequests() {
  ...
  const { data: requests, isLoading } = useQuery<AccountRequest[]>({
    queryKey: ["/api/ministry-admin/account-requests"],
  });
  ...
  const approveMutation = useMutation({
    mutationFn: async (data: ReviewFormData) => {
      const requestData = {
        ...data,
        churchName: reviewingRequest?.churchName || "",
        churchId: reviewingRequest?.churchId,
      };
      const res = await apiRequest("POST", `/api/ministry-admin/account-requests/${reviewingRequest?.id}/approve`, requestData);
      return res.json();
    },
    onSuccess: (data) => {
      if (data.credentials) {
  ...
  const denyMutation = useMutation({
    mutationFn: async () => {
      await apiRequest("POST", `/api/ministry-admin/account-requests/${reviewingRequest?.id}/deny`);
    },
    onSuccess: () => {
      toast({
    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Review and approve leader account requests for your ministry</p>
        </div>
        ...
        {processedRequests.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Previously processed leader account requests
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
