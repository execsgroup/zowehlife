...
- **Interactive Dashboard & Reporting**: Admin and Ministry Admin dashboards feature interactive charts and CSV export (Leader dashboard shows stats, follow-ups, and registration links only — no charts). Uses `recharts` library. Reusable chart components in `client/src/components/dashboard-charts.tsx`. Reporting API endpoints at `/api/ministry-admin/reports/growth`, `/api/ministry-admin/reports/status-breakdown`, `/api/ministry-admin/reports/followup-stages`, `/api/ministry-admin/reports/checkin-outcomes`, `/api/ministry-admin/reports/leader-performance`. Charts include: growth trend area charts, status breakdown donut charts, follow-up stage bar charts, check-in outcome bar charts, leader performance table. Fully translated (en/es/fr/pt).
